...
LOG_PATH = './log/' # log파일 작성하기

FONT = 'HMKMRHD.TTF' # 폰트는 휴먼둥근헤드라인 보통
...
def find_overlapping_ranges(ranges):
    ...
    for i in range(len(ranges)):
        for j in range(i+1, len(ranges)):
            ...
            if (range1[1] <= range2[0] and range1[0] >= range2[1]) and (range1[3] <= range2[2] and range1[2] >= range2[3]):
...
now = datetime.now() # 나중 log파일 작성을 위한 시간
...
for i in range(file_length):
    ...
    for j in range(1,len(name_location_word_font_color_list[i])):
        ...
        name_location_word_font_color_list[i][j].append((color[2],color[1],color[0])) # 색상 정보 저장

current_datetime = now.strftime('%Y-%m-%d-%H-%M')  # 현재 날짜와 시간을 '년도-월-일-시간' 형식으로 변환
log_file_name = f'{current_datetime}.txt'  # 로그 파일 이름 생성
f = open(os.path.join(LOG_PATH, log_file_name),'w')
for i in range(file_length):
    location = []
    for j in range(1,len(name_location_word_font_color_list[i])):
        location.append(name_location_word_font_color_list[i][j][0])
    indexs = find_overlapping_ranges(location) # 겹치는 영역에 대한 인덱스를 찾음
    for k,l in indexs:
        name_location_word_font_color_list[i][l+1][3] = name_location_word_font_color_list[i][k+1][3] # 겹치는 영역을 가진 인덱스의 색상을 같게 맞춰줌
    f.write(str(name_location_word_font_color_list[i]) + '\n') # 로그파일 작성
f.close()    
