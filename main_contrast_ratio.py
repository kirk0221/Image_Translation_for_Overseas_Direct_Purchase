...
def compare_pixels(pixel1, pixel2):
    ...
    return diff / 3 >= 80 # 모든 차이 값의 평균이 80 이상 차이나면 True 반환
    ...
    for j in range(1,len(name_location_word_font_color_list[i])):
        ...
        for k in range(int(x_max - x_min - 10)): # 픽셀 x축 이동
            for l in range(int(y_max - y_min - 10)): # 픽셀 y축 이동
                ...
                if compare_pixels(original_pixel_color, background_pixel_color) : # 오차값으로 비교해서 그보다 더 큰 경우에만
        ...
        if len(counts.most_common()) == 0: # 만약 최빈값이 안나오면 기본값은 검은색임
            ...
            c = 0
            while(True):
                if len(counts.most_common()) == c:
                    color = [0,255,0]
                elif len(counts.most_common())+1 == c:
                    color = [0,0,255]
                elif len(counts.most_common())+2 == c:
                    color = [255,0,0]
                elif len(counts.most_common())+3 == c:
                    color = [255,255,255]
                elif len(counts.most_common())+4 <= c:
                    color = [0,0,0]
                    break
                else:
                    color = list_most_common(counts, c)
                
                if contrast_ratio(color, background_color) > 3: # 대비 비율이 WCAG 1.4.3기준인 4.5:1을 넘는지 확인 -> 더 좋은 가독성을 위해
                    break
                c+=1
